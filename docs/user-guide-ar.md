...
ملاحظة مهمة:
- الاختصارات مثل `Ctrl+V`, `Ctrl+C`, `Alt+Tab` يتم تسجيلها الآن كـ `hotkey` وإعادة تنفيذها بشكل صحيح.
- إذا كانت لديك sessions قديمة تم تسجيلها قبل هذا الإصلاح، يفضل إعادة تسجيلها للحصول على نتائج أدق.
...
- يمكنك إيقاف Replay فورًا أثناء التشغيل بالضغط على `Esc`.
- ملخص replay يحتوي الآن على `stopped_by_user` لمعرفة هل التوقف كان يدويًا أم لا.
